...
Open the helmet, select a SIM and data bundle, then hover over a daily bar to inspect its date, amount, and status in the main usage section. Click the bar or activate the summary with the keyboard to open the adjacent detail panel on that day. The panel groups selected-day details, current-cycle totals and estimate, and data scope. Hover its chart to select another day. Arrow keys select a day; Close or Escape dismisses the panel. **Refresh** refreshes balance first and schedules optional history. **Data units** in Settings changes both chart units.
...
The gate first runs bundled `vikingbar proof history-api`. This forces real daily reads and compares their mapping and forecast arithmetic independently. It then records the cached balance timestamp, launches the fresh native bundle, and waits for a newer successful launch refresh. Main-bar hover on `vikingbar.historyMainPlot` must show exact day details without opening the panel. A real click on that bar must open the panel with the same selected day. The main AXPopover and the separate `vikingbar.historyPanel` must each match a visible CG window on a real display. The chart, total, forecast, status, and scope must fit fully inside the companion window and match the same production presentation. Negative-origin displays are supported. Native Refresh must produce a newer successful balance; eligible historical samples may come from the scoped cache.
...
- On 15 September 2026, code head `8645afe` passed the complete stored-session gate: 30 real requests, 18 observed cycle days, API/forecast mapping, main-bar hover without opening details, click-open selection preservation, full chart values, exact captures, newer native Refresh, and real Quit cleanup. The private run is `04928debe28843e595c37408487d6cc4`. App SHA-256 is `f1936a7fff793d831e6bec5e7d3d0350b89e33ed4eb660f25e09ed239356bdec`; bundled CLI SHA-256 is `f5f99e89a9d33b0cdb7c97a0591ca2523a8b4c95d84721f319d0cd29062eda2c`. Earlier AX-series, focus, and launch-refresh failures remain covered by the exact comparator, synthetic keyboard checks, and strict pre-launch timestamp baseline. No password or 1Password read is part of this workflow.
