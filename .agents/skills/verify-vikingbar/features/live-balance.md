...
On main `a33d6a89eeda0ffe566bf2d8bc52476ec3b6940b`, fresh native reconnect, debug API comparison and token rotation, Refresh, and stored-session debug relaunch passed. The first release-relaunch attempt timed out. A fresh release stored-session retry then passed with the same connection identity and zero additional credential reads. It verified picker label and selection, API comparison, Refresh, and native Quit. The coordinator inspected the actual card PNGs and confirmed app/worker cleanup. This retry completes release coverage for that main revision; it does not turn the earlier timeout into a passing attempt.
