...
export function App() {
  ...
  return <div className={motion ? 'site motion-on' : 'site'}>
    ...
    <dialog className="get-dialog" aria-labelledby="download-title" ref={getDialog} onClick={e => {if (e.target === e.currentTarget) e.currentTarget.close();}}><button className="close-dialog" onClick={() => getDialog.current.close()} aria-label="Close download details">Close <span aria-hidden="true">×</span></button><span className="eyebrow">VikingBar for Mac</span><h2 id="download-title">Try the<br/>unsigned preview.</h2><p>The free unsigned preview runs on Apple Silicon Macs with macOS 14 or later. It is not Developer ID signed or notarized. Download it from GitHub without an account.</p><p>If macOS blocks the first launch, open System Settings, choose Privacy &amp; Security, then click Open Anyway. Updates may require approval again.</p><p>First request API access by emailing <a href="mailto:api@mobilevikings.be">api@mobilevikings.be</a>. Wait for approval and your public client ID. Sign in directly in VikingBar, or use the optional configured 1Password helper. No client secret is required.</p><a className="primary" href={`${REPO}/releases/tag/v0.1.0`} target="_blank" rel="noreferrer">Download unsigned preview <span aria-hidden="true">↗</span></a><a className="setup-link" href={`${REPO}/blob/main/docs/RELEASING.md#install-the-unsigned-preview`} target="_blank" rel="noreferrer">Installation instructions <span aria-hidden="true">↗</span></a><a className="setup-link" href={`${REPO}/blob/main/docs/live-account.md`} target="_blank" rel="noreferrer">Account setup <span aria-hidden="true">↗</span></a></dialog>
