...
On 15 September 2026, build `7300211` passed the live API and forecast comparison after the earlier Keychain wait was resolved. Its native gate found that the chart's complete daily series was absent from the accessibility tree; cleanup passed. Earlier synthetic hover checks did not cover that assertion. The compact revision exposes the series through the chart label, and the hardened synthetic harness now invokes the unchanged production comparator. Build `d473579` subsequently passed the 30-request live API comparison. Its native gate raced the launch refresh against an older expected report; cleanup passed. The gate now requires a balance newer than the pre-launch snapshot before testing interactions. A complete passing live gate for main-bar hover and click-open details is still required before issue completion. See the [history feature map](../.agents/skills/verify-vikingbar/features/history.md) for commands, evidence, and coverage boundaries.
